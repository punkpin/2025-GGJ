...
public class ShadowMoveController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.V)) // J 键向左
        {
            moveX = -5f;
        }
        else if (Input.GetKey(KeyCode.N)) // L 键向右
        {
            moveX = 5f;
        }

        if (Input.GetKey(KeyCode.G)) // I 键向上
        {
            moveY = 5f;
        }
        else if (Input.GetKey(KeyCode.B)) // K 键向下
        {
            moveY = -5f;
        }
    ...
    void UpdateAnimationState(float moveX, float moveY)
    {
        ...
        else
        {
           ...
           // animator.SetBool("MoveRight", lastDirection == "MoveRight");
        }
    }
